...
class GoodController extends Controller
{
        ...
        if (isset($input['query'])) {
            ...
            foreach ($keywords as $index => $keyword) {
                if ($keyword[0] == '-') {
                    ...
                    $wheredata[$index] = ['good_name', 'not like', "%$keyword%"];
                } else {
                    $wheredata[$index] = ['good_name', 'like', "%$keyword%"];
                }
            }
            if (count($keywords) > 0) {
    ...
    public function addGood(AddGoodRequest $request)
    {
        ...
        Storage::put(
            ...
            Image::make($request->file('goodTitlePic'))->crop(round($input['crop_width']), round($input['crop_height']),
                round($input['crop_x']), round($input['crop_y']))->resize(800, 450)->encode('data-url')
        );
    ...
    public function editGood(EditGoodRequest $request, $good_id)
    {
        ...
        if ($request->hasFile('goodTitlePic')) {
            Storage::put(
                'good/titlepic/'.sha1($good->id),
                Image::make($request->file('goodTitlePic'))->crop(round($input['crop_width']),
