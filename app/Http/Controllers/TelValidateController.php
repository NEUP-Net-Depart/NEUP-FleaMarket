...
class TelValidateController extends Controller
{
        ...
        if (env('APP_ENV') != 'testing') {
            ...
            if ($request->session()->has('captcha.timestamp')) {
                if (time() - intval($request->session()->get('captcha.timestamp')) <= intval(env('MSG_COOLDOWN')))
                    return json_encode(array('rst' => 'false', 'msg' => '重试间隔过短，请稍等'));
            }
            ...
            $mobile = $request->tel;
            $text = view('auth.textCaptcha')->with(array(
                'code' => $captcha
            ))->render();
            ...
            try {
                ...
                if ($result['code'] == 0) {
                    ...
                    return json_encode(array('rst' => 'true', 'msg' => $result['msg']));
                } else
                    return json_encode(array('rst' => 'false', 'msg' => $result['msg']));
            } catch (Exception $exception) {
                return json_encode(array('rst' => 'false', 'msg' => '服务器错误'));
            }
        } else {
            $request->session()->put('captcha.tel', $request->tel);
            return json_encode([]);
    ...
    public function sendRegTextCaptcha(Request $request)
    {
        ...
        if (User::where('tel', $request->tel)->count() > 0)
            return json_encode(array('rst' => 'false', 'msg' => '手机已存在'));
    ...
    public function saveUserTel(Request $request)
    {
        ...
        $user->save();
        return json_encode(array('rst' => 'true', 'msg' => '手机绑定成功'));
    }
}
