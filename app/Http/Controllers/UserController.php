...
class UserController extends Controller
{
    ...
    public function completeUser(Request $request)
    {
        ...
        if ($request->hasFile('avatarPic')) {
            Storage::put(
                'avatar/'.$user->id,
                Image::make($request->file('avatarPic'))->crop(round($input['crop_width']),
                    round($input['crop_height']), round($input['crop_x']), round($input['crop_y']))->resize(800,
                    450)->encode('data-url')
            );
        }
    ...
    public function editUserInfo(Request $request, $userinfo_id)
    {
        ...
        $user_id = $request->session()->get('user_id');
        if ($data['userinfo'][0]->user_id != $user_id) {
            return Redirect::to('/user/'.$user_id)->withErrors('无权限访问');
        }
    ...
    public function userProfile(Request $request, $user_id)
    {
        ...
        $data['user'] = $user;
        $data['goods'] = GoodInfo::where('user_id', $user_id)->where('count', '>', 0)->where('baned',
            false)->paginate(15);
    ...
    public function mygoods_on(Request $request)
    {
        $data = [];
        $user_id = $request->session()->get('user_id');
        $data['goods'] = GoodInfo::where('user_id', $user_id)->where('count', '>', 0)->orderby('id',
            'desc')->paginate(15);
    ...
    public function buyer(Request $request)
    {
        $data = [];
        $user_id = $request->session()->get('user_id');
        $data['trans'] = Transaction::with('feedback', 'good')->where('buyer_id', $user_id)->orderBy('id',
            'desc')->paginate(15);
