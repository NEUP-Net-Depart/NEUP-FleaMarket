...
class User extends Model
{
    ...
    public function registerCompletion($force = false)
    {
        if ($force) {
            ...
            if ($this->nickname == null || !Storage::exists('avatar/'.$this->id)) {
                ...
                if ($this->wechat_open_id == null && $this->tel == null) {
                    return 3;
    ...
    public function getUserRankAttribute()
    {
        if (!$this->stuid) {
            ...
            if (strlen($this->stuid) == 8) {
                if (2000 + env('FRESH_YEAR') - intval(substr($this->stuid, 0, 4)) == 0) {
                    return "Freshman";
                } else {
                    if (2000 + env('FRESH_YEAR') - intval(substr($this->stuid, 0, 4)) == 1) {
                        return "Sophomore";
                    } else {
                        if (2000 + env('FRESH_YEAR') - intval(substr($this->stuid, 0, 4)) == 2) {
                            return "Junior";
                        } else {
                            if (2000 + env('FRESH_YEAR') - intval(substr($this->stuid, 0, 4)) == 3) {
                                return "Senior";
                            } else {
                ...
                if (strlen($this->stuid) == 7) {
                    if ($this->stuid[2] == '1' && env('FRESH_YEAR') - intval(substr($this->stuid, 0, 2)) < 5) {
                        return "Doctor";
                    } else {
                        if (($this->stuid[2] == '0' || $this->stuid[2] == '7') && env('FRESH_YEAR') - intval(substr($this->stuid,
                                0, 2)) < 3) {
                            return "Master";
                        } else {
                            return "Graduate";
