    ...
    <div class="col col-md-10 col-lg-9 col-xl-8 mx-auto">
      <div class="card">
        <div class="card-body">
          ...
          <form action=@if($add) "/good/add" @else "/good/{{$good->id}}/edit" @endif method="POST"
          enctype="multipart/form-data" id="add_form">
          <div class="form-group">
            <label for="good_name">商品名称</label>
            <input name="good_name" id="good_name" class="form-control"
                   value="{{count($errors) ? old('good_name') : $good->good_name}}" placeholder="商品名称">
          </div>
          <div class="row">
            <div class="col">
              <label>商品分类</label>
          ...
          <div class="row">
            <div class="col">
              <select name="cat_id" class="btn btn-secondary dropdown-toggle w-100">
                @foreach($cats as $cat)
                  <option value="{{$cat->id}}"
                          @if(($good->cat_id==$cat->id&&!count($errors))||(old('cat_id')==$cat->id&&count($errors)))  selected="selected" @endif>{{$cat->cat_name}}</option>
                @endforeach
              </select>
            </div>
          </div>
            ...
            <div class="col">
              <div class="form-group">
                ...
                <textarea name="description" id="description" placeholder="商品描述" class="form-control"
                          style="resize:none;"
                          rows="6">{!! count($errors) ? old('description') : $good->description !!}</textarea>
            ...
            <div class="col">
              <div class="form-group">
                ...
                <input type="number" step="0.01" min="0" name="price" id="price"
                       value="{{count($errors) ? old('price') : $good->price}}" placeholder="商品价格" class="form-control">
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col">
              <div class="form-group d-none">
                ...
                <select name="type" class="btn btn-secondary dropdown-toggle" value="0">
                  <option value="0"
                          @if(($good->type==0&&!count($errors))||(old('type')==0&&count($errors))) selected="selected" @endif>
                    普通商品
                  </option>
                  <option value="1"
                          @if(($good->type==1&&!count($errors))||(old('type')==1&&count($errors))) selected="selected" @endif>
                    拍卖商品
                  </option>
                </select>
              </div>
            ...
            <div class="col">
              <div class="form-group">
                ...
                <input type="number" name="count" id="count" min="0"
                       value="{{count($errors) ? old('count') : $good->count}}" placeholder="库存" class="form-control"
          ...
          {!! csrf_field() !!}
          <div class="row">
            <div class="col-auto mr-auto"><input type="button" class="btn btn-primary" value="@if($add)添加@else更改@endif"
                                                 onclick="mb_upload()" /></div>
            <div class="col-auto ml-auto"><input type="button" class="btn btn-success" value="商品列表"
                                                 onclick="window.location.href=('/good')"></div>
          </div>
          </form>
        </div>
      </div>
