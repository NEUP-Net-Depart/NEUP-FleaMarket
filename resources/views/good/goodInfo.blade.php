          ...
          <div class="card">
            ...
            <table class="table table-hover" style="margin-bottom:0">
              ...
              <tr>
                <th>卖家</th>
                <td><a
                    href="/user/{{ $user->id }}">@if($user->nickname!=""&&$user->nickname!=NULL){{ $user->nickname }} @else
                      还没有昵称&gt;_&lt; @endif @if($user->baned)【已封禁】@endif</a></td>
              </tr>
              ...
              <tr>
                <th>库存</th>
                <td
                  @if($good->count==0) class="text-danger" @endif>@if($good->count>0) @if($good->count > 1){{ $good->count }}
                  件@else 仅一件 @endif @else 没库存了QAQ @endif</td>
              </tr>
              <tr>
                <th>分类</th>
                <td>
                  @if(Session::get('is_admin')>=1 && !$good->baned)
                    <form action="/good/{{$good->id}}/updateCat" method="POST">
                      {!! csrf_field() !!}
                      <select name="cat_id" id="cat_id" class="btn btn-secondary dropdown-toggle btn-sm"
              ...
              <tr>
                ...
                <td>
                  @if(isset($inFvlst))
                    @if(count($inFvlst) == 0)
                      <button class="fa fa-star-o btn btn-primary btn-sm" onclick="add_favlist()" data-toggle="tooltip"
                              data-placement="top" title="收藏OvO"></button>
                    @endif
                    @if(count($inFvlst) != 0)
                      <button class="fa fa-star btn btn-primary btn-sm" onclick="del_favlist()" data-toggle="tooltip"
                              data-placement="top" title="取消收藏QAQ"></button>
                    @endif
                  @else
                    <button class="fa fa-star-o btn btn-primary btn-sm" onclick="window.location.href='/login'"
                            data-toggle="tooltip" data-placement="top" title="收藏OvO"></button>
                  @endif
                </td>
              </tr>
              <tr>
