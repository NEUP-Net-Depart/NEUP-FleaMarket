...
@section('content')
        ...
        <div class="carousel-inner">
          ...
          @foreach($stargoods as $good)
            <div class="carousel-item @if($loop->index == $stargoodfirstid) active @endif ">
              <a href="/good/{{ $good->id }}"><img class="d-block w-100" src="/good/{{ sha1($good->id) }}/titlepic"
                                                   alt="{{ $good->good_name }}"></a>
            </div>
          @endforeach
        </div>
  ...
  <div class="row mx-auto d-md-none" style="margin-top:12px;height:65px;overflow:hidden">
    <div class="row">
      <div
        style="overflow-x: scroll;overflow-y: hidden;height:133px;white-space: nowrap;display:-webkit-box;display:-moz-box;">
        @foreach($populargoods as $good)
    ...
    <div class="col">
      ...
      @foreach($cats as $cat)
        @if(count($catgoods[$cat->cat_name]))
          <p><h4 class="d-inline">{{ $cat->cat_name }}</h4>&nbsp;&nbsp;&nbsp;<a href="/good?cat_id={{ $cat->id }}"><h6
              class="d-inline">更多</h6></a></p>
          <div class="row">
