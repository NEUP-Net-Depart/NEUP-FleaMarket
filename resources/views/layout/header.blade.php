    ...
    <div class="dropdown">
      ...
      <div class="dropdown-menu dropdown-menu-right">
        @if(Session::has('user_id'))
          ...
          <a href="/logout" class="dropdown-item bg-danger text-light">
            <center>登出</center>
          </a>
          ...
          <div class="dropdown-divider"></div>
          <a href="/message" class="dropdown-item">消息 <span class='badge badge-warning message-num-tip'
      ...
      <ul class="navbar-nav ml-auto">
        @if(Session::has('user_id'))
          <li class="nav-item dropdown">
            <a class="nav-link navbar-avatar nav-btn" href="#" data-toggle="dropdown" role="button" aria-haspopup="true"
               aria-expanded="false"><img src="/avatar/{{ Session::get('user_id') }}/40/40" class="avatar"
          ...
          <li class="nav-item"><a href="/message" class="nav-link nav-btn">消息 <span
                class='badge badge-warning message-num-tip' style='display: none'></span></a></li>
          <li class="nav-item"><a href="/good/add" class="nav-link nav-btn">出售</a></li>
        @else
          <li class="nav-item dropdown">
            <a class="nav-link nav-btn" href="#" data-toggle="dropdown" role="button" aria-haspopup="true"
      ...
      <form action="/good" method="GET">
        <div class="input-group">
          <input class="form-control" name="query" id="searchq" placeholder="开始交易吧( '﹃'⑉)"
                 value="@if(isset($_GET['query'])){{ urldecode($_GET['query']) }}@endif">
...
<div class="container classbar-container">
  <ul class="nav nav-pills classbar-pills">
    <li class="nav-item"><a href="/good"
                            class="nav-link classbar-btn @if(isset($cat_id)&&$cat_id == 0) classbar-btn-active @endif">所有商品</a>
    </li>
    @foreach($cats as $cat)
      <li class="nav-item"><a href="/good?cat_id={{ $cat->id }}"
                              class="nav-link classbar-btn @if(isset($cat_id) && $cat_id == $cat->id) classbar-btn-active @endif">{{ $cat->cat_name }}</a>
      </li>
    @endforeach
  </ul>
</div>
