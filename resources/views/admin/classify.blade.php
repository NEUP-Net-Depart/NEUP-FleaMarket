  ...
  <div class="tab-pane active" id="classify" role="tabpanel">
    <template v-for="cat in cats">
      <div class="container form-control">
        <div class="row">
          <div class="col">
            <h3><input type="checkbox" name="tag" v-bind:value="cat.id" v-model="checkedCat.cats">@{{ cat.cat_name }}
            </h3>
          </div>
        </div>
        <hr>
    ...
    <div class="container form-control">
      ...
      <div class="form-inline">
        ...
        <div class="mx-sm-1">
          <input class="form-control" v-bind:placeholder="checkedCat.cats.length <= 1 ? '重命名' : '合并并更名'"
                 v-model="checkedCat.cat_name">
        </div>
        <div class="mx-sm-3">
          <button class="btn btn-primary" v-on:click="editCat">@{{ checkedCat.cats.length <= 1 ? '修改' : '合并分类' }}
          </button>
        </div>
      </div>
    </div>
  ...
  <script type="text/x-template" id="tag">
    <div>
      ...
      <div class="form-inline">
        ...
        <div class="mx-sm-1">
          <input class="form-control" v-bind:placeholder="checkedTag.tags.length <= 1 ? '重命名' : '合并并更名'"
                 v-model="checkedTag.tag_name">
        </div>
        <div class="mx-sm-3">
          <button class="btn btn-primary" v-on:click="editTag">@{{ checkedTag.tags.length <= 1 ? '修改' : '合并 Tag' }}
          </button>
        </div>
      </div>
    </div>
