      ...
      <form action="/user/edit/email" method="POST" class="password-form">
        ...
        <div class="input-group">
          <input type="email" name="email" id="email" class="form-control" value="{{$user->email}}" placeholder="邮箱"
                 required>
          <span class="input-group-btn"><input type="submit" class="btn btn-primary" name="email_submit"
                                               value="绑定"></span>
        </div>
      </form>
    @else
      <form action="/user/edit/email" method="POST" class="password-form">
        ...
        <div class="input-group">
          <input type="email" id="email" class="form-control"
                 value="@if(!$user->havecheckedemail)[未验证]@endif{{$user->email}}" disabled>
...
<div class="row">
  <div class="col-10 mx-auto">
    ...
    @else
      <div class="input-group">
        <input placeholder="用户名" type="text" name="username" id="username" class="form-control"
      ...
      @if($user->stuid=='')
        <form action="/user/edit/stuid" method="POST" class="password-form">
          {!! csrf_field() !!}
          <div class="input-group">
            <input placeholder="学号" type="text" name="stuid" id="stuid" class="form-control" value="{{$user->stuid}}"
                   required>
            <span class="input-group-btn"><input type="submit" class="btn btn-primary" name="stuid_submit"
                                                 value="绑定"></span>
          </div>
        </form>
      @else
        <div class="input-group">
          <input placeholder="学号" type="text" name="stuid" id="stuid" class="form-control" value="{{$user->stuid}}"
                 disabled>
          <span class="input-group-btn"><input type="submit" class="btn btn-secondary" name="stuid_submit" value="绑定"
                                               disabled></span>
        </div>
        @endif
        </p>
