  ...
  <form action="/user/fav/del" method="POST">
    ...
    <table class="table">
      ...
      @foreach($goods as $good)
        <tr>
          <td>{{ $good->good_id }}</td>
          <td><a href="/good/{{$good->good_id}}"
                 onMouseOver="toolTip('<img src=/good/{{ sha1($good->good_id) }}/titlepic>')"
                 onMouseOut="toolTip()">{{ $good_info[$good->good_id]->good_name }}</a><img
              src="/good/{{ sha1($good->good_id) }}/titlepic" class="pic" /></td>
